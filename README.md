

# 🌟 Skills and Contributions

Welcome to my portfolio! Lately I tend to focus in game theory, algorithmic solutions, machine learning, and prompt engineering to address complex challenges.

<p align="center">
  <img src="peacechess.webp" width="50%" alt="Descripción">
</p>


## Game Theory and Strategic Decision-Making


- [PeaceChess](https://github.com/MrCabss69/PeaceChess): chess engine with a-b prunning and Zobrist hashing / MCTS, tree-search and optimization.


## Machine Learning
- [Gymnasium](https://github.com/MrCabss69/Gymnasium): dedicated to applying reinforcement learning in various settings.


- [Machine-Learning-Blackjack](https://github.com/MrCabss69/Machine-Learning-Blackjack): employing neural networks, reinforcement learning, and genetic algorithms for optimizing Blackjack strategies.





## Additional Projects

### Prompt engenieering tools

- [RepoGPT](https://github.com/MrCabss69/RepoGPT): repository summarization and processing tool, preparing files for input in your preferred format for LLMs.

- [TikTokenCost](https://github.com/MrCabss69/TikTokenCost): Leverages tiktoken cost utility by calculating the exact cost of API usage, based on OpenAI prices.

### Scraping, wrappers, web and apps

- [Python-Catastro](https://github.com/MrCabss69/Python-Catastro): A wrapper for straightforward Catastro API access via Python.
- [Laravel and Python TicTacToe](https://github.com/MrCabss69/TicTacToe): A web-based Tic Tac Toe game.
- [MiniFlopZilla](https://github.com/MrCabss69/MiniFlopZilla): A Tkinter UI for calculating poker odds.
- [Golden Geometry](https://goldengeometry.com) and [@geometria_aurea](https://www.instagram.com/geometria_aurea/)
